---
openapi: '/openapi.json GET /v1/token/price'
title: "Token Price"
description: "Cheap price tier for bots and tickers: price, market cap, liquidity. Nothing else."
---

`GET /v1/token/price` · **1 credit**

Cheap price tier for bots and tickers: price, market cap, liquidity. Nothing else.

<Note>`priceNative`, `priceUsd`, `marketCapUsd` and `liquidityUsd` are `null` when the token has not traded yet. A token whose price fell to zero after trading keeps `priceNative` `"0"`.</Note>

<RequestExample>
```bash cURL
curl 'https://api.serialized.xyz/v1/token/price?chain=evm:8453&address=0x4ed4e862860bed51a9570b96d89af5e1b0efefed' \
  -H 'Authorization: YOUR_API_KEY'
```
</RequestExample>

<ResponseExample>
```json Response
{
  "data": {
    "chain": "evm:8453",
    "address": "0x4ed4e862860bed51a9570b96d89af5e1b0efefed",
    "priceNative": "0.00000054747124708872",
    "priceUsd": 0.0010292322577456162,
    "marketCapUsd": 37389232.09806793,
    "liquidityUsd": 239386.56038744663
  },
  "meta": {
    "asOf": 1786724320112
  }
}
```
</ResponseExample>
