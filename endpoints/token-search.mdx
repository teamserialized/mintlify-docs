...
<Note>`pool` is the token's best market under the canonical priority ranking: the same pool [`/v1/token`](/endpoints/token-details) serves and the first row of [`/v1/token/pools`](/endpoints/token-pools). `pool.factory` is the launchpad key of the registry, the same value as on `/v1/token` and `/v1/pulse`.</Note>
