...
| Native coin as `0xeeee…eeee`                                    | native coin = `0x0000…0000` on EVM (`pool.quote.address`, `token/pools`), `So11111111111111111111111111111111111111112` on Solana | the chain's own convention, no synthetic address |
...
| `GET /api/2/market/details` (windowed stats)       | `GET /v1/token/stats`                                    | token-wide by default, `pools=` to restrict; `5m`, `1h`, `6h`, `24h` by default (same on `/v1/pool/data`); per-window `txns`, buy/sell split and `priceChangePct` |
