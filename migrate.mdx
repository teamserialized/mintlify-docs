...
| `GET /api/2/token/market-details` (windowed stats) | `GET /v1/token/stats?windows=5m,1h,24h`                  | token-wide by default, `pools=` to restrict; per-window `txns`, buy/sell split and `priceChangePct` |
