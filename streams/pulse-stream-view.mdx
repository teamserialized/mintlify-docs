---
title: "Pulse Stream (view)"
description: "A live launchpad view: the server maintains the filtered, sorted view and pushes it - a full `snapshot` on subscribe, then `enter`/`update`/`leave` deltas as tokens move through the lifecycle. Cards are the exact REST `/v1/pulse` shape."
---

`WS channel: pulse`

A live launchpad view: the server maintains the filtered, sorted view and pushes it - a full `snapshot` on subscribe, then `enter`/`update`/`leave` deltas as tokens move through the lifecycle. Cards are the exact REST `/v1/pulse` shape.
