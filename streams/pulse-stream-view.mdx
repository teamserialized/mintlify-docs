...
<ParamField body="chains" type="string" required>
  Comma-separated public chain ids (≤8), EVM and Solana mixable in one subscription. Example: `evm:8453,solana`
</ParamField>
...
<Note>Close codes: `4401` (auth - key missing, invalid or revoked; a revoke also closes open connections), `4402` (monthly credit quota exhausted, the WS mirror of REST `402`), `1008` (policy - connection/subscription limits, frame floods, a stalled socket), `1001` (idle timeout - send `{"op":"ping"}` every 30s).</Note>
